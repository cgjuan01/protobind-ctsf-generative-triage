...
**novelty filter against the real cathepsin chemical space**, and the
**sequence-form comparison**.
...
- **Sequence form matters, decisively.** The two forms produced **entirely
  non-overlapping** molecule sets (Jaccard = 0.000), and the **mature catalytic
  domain outperformed the full preproprotein on every axis** (drug-likeness, Lipinski
  rate, MW, logP, and drug-like-novel yield). Conditioning on the domain where ligands
  actually bind yields measurably better candidates — a small but real methodological
  observation.
...
- This work: triage, novelty filtering, and sequence-form comparison — for research /
